...
function displayBooks() {
  ...
  myLibrary.forEach((book, index) => {
    ...
    book.read
      ? read.classList.add("current-read-status")
      : unread.classList.add("current-read-status");
    ...
    unreadButtons.forEach((button) =>
      button.addEventListener("click", setUnread)
    );
  });
}
...
function setRead() {
  let book =
    myLibrary[
      this.parentElement.parentElement.parentElement.getAttribute("data-index")
    ];
  book.setRead();
  displayBooks();
}
...
let hitchhikersGuideToTheGalaxy = new Book(
